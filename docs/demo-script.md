...
> Demo script for the 3-minute submission video. The 5-beat structure is
> Problem → Solution → App demo → Technical implementation → Closing.
> Voiceover is generated by Chirp 3; the side-by-side signature moment is
> the visual proof of cross-shot consistency.

## Format 

- **Length:** 3 minutes (≤ 3:00 hard cap).
...
- **Default landing:** the deployed app loads the sample
  ...
  triggered by the "Watch it live" button.
...
Visual: Research Panel. Each query
appears as it's sent. Results stream in with their URLs visible — this is
the transparency guarantee: a viewer watching the video sees the partner
API being called live, not a stubbed response.
...
Visual: Pydantic models slide past (`CharacterSpec`,
...
For social and press use, produce shorter cuts from this script:
30-second (Twitter/X hook), 60-second (LinkedIn / trailer), 90-second
(first-impression cut), 5-minute (deep-dive — adds schema walkthrough,
a second logline to prove generalization, the learning-loop discussion,
the roadmap, and the open-source strategy).
