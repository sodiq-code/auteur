...
> Source: blueprint Section 23.2 (Pydantic schema), 23.3 (versioning &
...
## Why the Bible is typed (blueprint Section 23 preamble)
...
## Schema (blueprint Section 23.2, Pydantic)

The Python source below is reproduced verbatim from blueprint P613. The
...
## Versioning & Citation (blueprint Section 23.3)
...
1. **Every user edit creates a new Bible version** (immutable snapshot). v1
   ...
   `bibles/{projectId}/{version}` (blueprint Table 33, Row 2).
2. **Every generation cites which Bible version it used.** The
   ...
   (blueprint Table 33, Row 4). This is the row-level citation.
3. **Drift is therefore attributable.** A drift report cites its
   ...
   generation → drift report (blueprint Section 24.5).
...
## Provenance chain (blueprint Section 24.5)
...
## Learning Loop (blueprint Section 23.4)
...
3. **Context injection.** Re-generation prompts tighten. Each
   ...
   re-generations within a project (blueprint Section 22.5 — "Measurable
   improvement").

## Validation

All inputs are validated with Pydantic schemas (fail-fast — blueprint
