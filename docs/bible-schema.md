...
> Film Bible schema reference. The schema is implemented in
...
RAG. Cross-project learning is a post-hackathon roadmap item.)
...
3. **Context injection.** Re-generation prompts tighten. Each
   ...
   re-generations within a project.

## Validation

All inputs are validated with Pydantic schemas (fail-fast). The
Pydantic models above are the single source of truth; the TypeScript types
in `frontend/src/lib/types.ts` mirror them, and the OpenAPI schema
generated by FastAPI at `/openapi.json` is the wire contract.
